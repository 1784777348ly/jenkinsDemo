...
@property (strong, nonatomic) UIImageView *leftBack;
@property (strong, nonatomic) UIButton *rightCustom;
...
- (void)setNavigation{
    ...
    [self setSystemBar:@"手机相册" rightTitle:@"提交"];
}
- (void)setSystemBar:(NSString *)title rightTitle:(NSString *)rightTitle{
    
    // 右侧button
    if (rightTitle) {
        [self.rightCustom setTitle:rightTitle forState:UIControlStateNormal];
        UIBarButtonItem * spaceItem = [[UIBarButtonItem alloc]initWithBarButtonSystemItem:UIBarButtonSystemItemFixedSpace target:nil action:nil];
        spaceItem.width = -10;
        UIBarButtonItem *right = [[UIBarButtonItem alloc] initWithCustomView:self.rightCustom];
    ...
    spaceItemLeft.width = -5;
    UIBarButtonItem *left = [[UIBarButtonItem alloc] initWithCustomView:self.leftBack];
    self.navigationItem.leftBarButtonItems = @[spaceItemLeft,left];
    //标题
    self.navigationItem.title = title;
    //底部橘色横线
    [self.navigationController.navigationBar addSubview:self.lineImg];
}
- (UIImageView *)leftBack{
    if (!_leftBack) {
        _leftBack = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"common_backarrow"]];
        UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(backAction)];
        [_leftBack addGestureRecognizer:tap];
        _leftBack.userInteractionEnabled = YES;
    }
    return _leftBack;
}
- (UIButton *)rightCustom{
    if (!_rightCustom) {
        _rightCustom = [UIButton buttonWithType:UIButtonTypeCustom];
        _rightCustom.frame = CGRectMake(0, 0, 75, 34);
        [_rightCustom setBackgroundImage:[UIImage imageNamed:@"recommend_follow_btn"] forState:UIControlStateNormal];
      //  [_rightCustom setTitleColor:white_Color forState:UIControlStateNormal];
        [_rightCustom addTarget:self action:@selector(okButtonClick) forControlEvents:UIControlEventTouchUpInside];
    }
    return _rightCustom;
}
- (UIImageView *)lineImg{
    if (!_lineImg) {
        ...
        _lineImg.frame = CGRectMake(0, 44, SCREEN_WIDTH, 1);
        _lineImg.backgroundColor = MainColor;
    }
    return _lineImg;
}
...
//    CGFloat itemWH = (self.view.tz_width - (self.columnNumber + 1) * margin) / self.columnNumber;
    CGFloat www = (SCREEN_WIDTH-margin*4)/3;
    layout.itemSize = CGSizeMake(www, www);
    layout.minimumInteritemSpacing = margin;
    layout.minimumLineSpacing = margin;
    CGFloat top = 44;
    if (iOS7Later) top += 20;
    CGFloat collectionViewHeight = tzImagePickerVc.maxImagesCount > 1 ? self.view.tz_height - top : self.view.tz_height - top;
...
- (void)didGetAllPhotos:(NSArray *)photos assets:(NSArray *)assets infoArr:(NSArray *)infoArr{
    TZImagePickerController *tzImagePickerVc = (TZImagePickerController *)self.navigationController;
    __weak typeof(tzImagePickerVc)weakTz = tzImagePickerVc;
...
//    if (weakTz.didFinishPickingPhotosHandle) {
//        weakTz.didFinishPickingPhotosHandle(photos,assets,_isSelectOriginalPhoto);
//    }
    
    if ([tzImagePickerVc.pickerDelegate respondsToSelector:@selector(didFinishPickPhoto:asset:isOriginal:)]) {
        
        [tzImagePickerVc.pickerDelegate didFinishPickPhoto:photos asset:assets isOriginal:_isSelectOriginalPhoto];
    }
    
    if (weakTz.didFinishPickingPhotosWithInfosHandle) {
        weakTz.didFinishPickingPhotosWithInfosHandle(photos,assets,_isSelectOriginalPhoto,infoArr);
    }
    [weakTz hideProgressHUD];
    
    if (weakTz.autoDismiss) {
        [self.navigationController dismissViewControllerAnimated:YES completion:nil];
    }
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (((tzImagePickerVc.sortAscendingByModificationDate && indexPath.row >= _models.count) || (!tzImagePickerVc.sortAscendingByModificationDate && indexPath.row == 0)) && _showTakePhotoBtn) {
        ...
        cell.imageView.image = [UIImage imageNamed:@"uploadimg_cambg"];
        return cell;
    }
